...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in woocode/packages/cli/package.json',
  );
}
...
console.log('packing woocode ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'woocode-*.tgz'), { force: true });
execSync(
  `npm pack -w woocode --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing woocode-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'woocode-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w woocode-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `woocode-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `woocode-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.WOOCODE_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
