  ...
  - **Sub-commands:**
    - **`save`**
      ...
      - **Details on Checkpoint Location:** The default locations for saved chat checkpoints are:
        - Linux/macOS: `~/.woocode/tmp/<project_hash>/`
        - Windows: `C:\Users\<YourUsername>\.woocode\tmp\<project_hash>\`
...
- **`/memory`**
  - **Description:** Manage the AI's instructional context (hierarchical memory loaded from `WOOCODE.md` files).
  - **Sub-commands:**
    - **`add`**:
      - **Description:** Adds the following text to the AI's memory. Usage: `/memory add <text to remember>`
    - **`show`**:
      - **Description:** Display the full, concatenated content of the current hierarchical memory that has been loaded from all `WOOCODE.md` files. This lets you inspect the instructional context being provided to the Gemini model.
    - **`refresh`**:
      - **Description:** Reload the hierarchical instructional memory from all `WOOCODE.md` files found in the configured locations (global, project/ancestors, and sub-directories). This command updates the model with the latest `WOOCODE.md` content.
    - **Note:** For more details on how `WOOCODE.md` files contribute to hierarchical memory, see the [CLI Configuration documentation](./configuration.md#4-geminimd-files-hierarchical-instructional-context).
...
- **`/settings`**
  ...
  - **Details:** This command provides a user-friendly interface for changing settings that control the behavior and appearance of Gemini CLI. It is equivalent to manually editing the `.woocode/settings.json` file, but with validation and guidance to prevent errors.
...
- **`/vim`**
  ...
  - **Features:**
    ...
    - **Persistent setting:** Vim mode preference is saved to `~/.woocode/settings.json` and restored between sessions
  - **Status indicator:** When enabled, shows `[NORMAL]` or `[INSERT]` in the footer

- **`/init`**
  - **Description:** To help users easily create a `WOOCODE.md` file, this command analyzes the current directory and generates a tailored context file, making it simpler for them to provide project-specific instructions to the Gemini agent.
...
1.  **User Commands (Global):** Located in `~/.woocode/commands/`. These commands are available in any project you are working on.
2.  **Project Commands (Local):** Located in `<your-project-root>/.woocode/commands/`. These commands are specific to the current project and can be checked into version control to be shared with your team.
...
- A file at `~/.woocode/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.woocode/commands/git/commit.toml` becomes the namespaced command `/git:commit`.
...
```toml
# In: <project>/.woocode/commands/changelog.toml
...
````toml
# In: <project>/.woocode/commands/git/commit.toml
...
- **Directory Listing**: `@{path/to/dir}` is traversed and each file present within the directory and all subdirectories are inserted into the prompt. This respects `.gitignore` and `.woocodeignore` if enabled.
...
```toml
# In: <project>/.woocode/commands/review.toml
...
```bash
mkdir -p ~/.woocode/commands/refactor
touch ~/.woocode/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.woocode/commands/refactor/pure.toml` in your editor and add the following content. We are including the optional `description` for best practice.

```toml
# In: ~/.woocode/commands/refactor/pure.toml
...
- **Environment Variable:** When a command is executed via `!` or in shell mode, the `WOOCODE_CLI=1` environment variable is set in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the Gemini CLI.
