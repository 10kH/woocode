...
2. User Settings (`~/.woocode/settings.json`)
3. Workspace Settings (`<project>/.woocode/settings.json`)
...
- **User `settings.json` (`~/.woocode/settings.json`):**
...
- **Workspace `settings.json` (`<project>/.woocode/settings.json`):**
...
- **Location**:
  ...
  - The path can be overridden using the `WOOCODE_CLI_SYSTEM_SETTINGS_PATH` environment variable.
