...
import {
  WOOCODE_CONFIG_DIR as WOOCODE_DIR,
  getErrorMessage,
  Storage,
} from 'woocode-core';
...
export const SETTINGS_DIRECTORY_NAME = '.woocode';
...
export function getSystemSettingsPath(): string {
  if (process.env['WOOCODE_CLI_SYSTEM_SETTINGS_PATH']) {
    return process.env['WOOCODE_CLI_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
...
export function getSystemDefaultsPath(): string {
  if (process.env['WOOCODE_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['WOOCODE_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    ...
    const geminiEnvPath = path.join(currentDir, WOOCODE_DIR, '.env');
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeGeminiEnvPath = path.join(homedir(), WOOCODE_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(WOOCODE_DIR);
