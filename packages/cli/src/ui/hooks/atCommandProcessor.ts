...
import type { AnyToolInvocation, Config } from 'woocode-core';
import {
  getErrorMessage,
  isNodeError,
  unescapePath,
...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const woocodeIgnored =
      respectFileIgnore.respectWoocodeIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectWoocodeIgnore: true,
      });

    if (gitIgnored || woocodeIgnored) {
      const reason =
        gitIgnored && woocodeIgnored ? 'both' : gitIgnored ? 'git' : 'woocode';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and woocode'
          : reason === 'git'
            ? 'git-ignored'
            : 'woocode-ignored';
  ...
  const totalIgnored =
    ...
    ignoredByReason['woocode'].length +
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['woocode'].length) {
      messages.push(`Woocode-ignored: ${ignoredByReason['woocode'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_woocode_ignore: respectFileIgnore.respectWoocodeIgnore,
    },
