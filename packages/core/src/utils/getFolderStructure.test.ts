...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile('.woocode', 'config.yaml');
      await createTestFile('.woocode', 'logs.json');
  ...
  describe('with geminiignore', () => {
    it('should ignore geminiignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.woocodeignore'),
        'ignored.txt\nnode_modules/\n.woocode/\n!/.woocode/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectWoocodeIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.woocodeignore'),
        'ignored.txt\nnode_modules/\n.woocode/\n!/.woocode/config.yaml',
      );
