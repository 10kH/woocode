...
export class Logger {
  private woocodeDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.woocodeDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.woocodeDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.woocodeDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.woocodeDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.woocodeDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.woocodeDir) {
      console.error(
